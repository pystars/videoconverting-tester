...
from ..options.models import AudioOptions, VideoOptions
from utils import *
...
def job_create(original_file, result_file, audio_options=None,
    ...
    if auto_size:
        width, height = autosize(
            job.original_file,
            job.video_options.max_width,
            job.video_options.max_height
        )
        job.video_options.size = '%dx%d' % (width, height)

    return job

def convert_file(original_file, result_file, *args, **kwargs):
